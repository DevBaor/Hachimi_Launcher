...
const hasValidRequirementHtml = (value?: string | null) => {
...
export function Sidebar() {
  ...
  const getRequirementValue = (requirementType: "minimum" | "recommended") =>
    [
      shopDetails?.pc_requirements?.[requirementType],
      shopDetails?.linux_requirements?.[requirementType],
      shopDetails?.mac_requirements?.[requirementType],
    ].find((value) => hasValidRequirementHtml(value));

  const getRequirementContent = (
    requirementType: "minimum" | "recommended"
  ) => {
    return (
      getRequirementValue(requirementType) ??
      t(`no_${requirementType}_requirements`, {
        gameTitle,
      })
    );
  };
  ...
  return (
    <aside className="content-sidebar">
      ...
      <SidebarSection title={t("requirements")}>
        {(["minimum", "recommended"] as const).map((requirementType) => (
          <section key={requirementType} className="requirement__section">
            <h4 className="requirement__title">{t(requirementType)}</h4>
            <div
              className="requirement__details"
              dangerouslySetInnerHTML={{
                __html: getRequirementContent(requirementType),
              }}
            />
          </section>
        ))}
