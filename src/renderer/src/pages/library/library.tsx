...
export default function Library() {
  ...
  const getGameKey = useCallback(
    (game: LibraryGame) => game.id ?? `${game.shop}:${game.objectId}`,
    []
  );
  ...
  const handleSelectAll = useCallback(() => {
    setSelectedGames(sortedLibrary.map((game) => getGameKey(game)));
  }, [sortedLibrary, getGameKey]);
  ...
  const handleConfirmDeleteSelected = useCallback(async () => {
    ...
    try {
      const selectedList = library.filter((game) =>
        selectedSet.has(getGameKey(game))
      );
      ...
      for (const game of selectedList) {
        if (lastPacket?.gameId === getGameKey(game)) {
      ...
      showSuccessToast(t("bulk_remove_success"));
      handleExitSelectionMode();
      setShowDeleteSelectedModal(false);
    } catch (error) {
      showErrorToast(t("bulk_remove_failed"));
    } finally {
  ...
  return (
    <section className="library__content">
                ...
                {selectionMode ? (
                  <>
                    <span className="library__bulk-count">
                      {t("bulk_selected", { count: selectedCount })}
                    </span>
                    <Button
                      theme="outline"
                    ...
                    <Button theme="outline" onClick={handleExitSelectionMode}>
                      {t("bulk_cancel")}
                    </Button>
                  </>
                ) : (
              ...
              >
                {sortedLibrary.map((game, index) => (
                  <LibraryGameCardLarge
                    ...
                    isSelected={selectedSet.has(getGameKey(game))}
      ...
      <ConfirmationModal
        ...
        title={t("bulk_remove_title")}
        descriptionText={t("bulk_remove_description", {
          count: selectedCount,
        })}
        onClose={() => {
        ...
        cancelButtonLabel={t("bulk_cancel")}
        confirmButtonLabel={t("bulk_remove")}
        buttonsIsDisabled={isDeletingSelected}
      />
    </section>
  );
}
