...
export class DeckyPlugin {
  ...
  private static async downloadPlugin(): Promise<string> {
    logger.log("Downloading Hachimi Decky plugin...");
  ...
  private static async extractPlugin(zipPath: string): Promise<string> {
    logger.log("Extracting Hachimi Decky plugin...");
  ...
  public static async checkAndUpdateIfOutdated(): Promise<void> {
    if (!fs.existsSync(HYDRA_DECKY_PLUGIN_LOCATION)) {
      logger.log("Hachimi Decky plugin not installed, skipping update check");
      return;
    }
    ...
    try {
      if (!fs.existsSync(packageJsonPath)) {
        logger.log(
          "Hachimi Decky plugin package.json not found, skipping update"
        );
        return;
      }
      ...
      if (isOutdated) {
        logger.log(
          `Hachimi Decky plugin is outdated. Current: ${currentVersion}, Expected: ${releaseInfo.version}. Updating...`
        );

        await this.updatePlugin();
        logger.log("Hachimi Decky plugin updated successfully");
      } else {
        logger.log(`Hachimi Decky plugin is up to date (${currentVersion})`);
      }
    } catch (error) {
      logger.error(`Error checking/updating Hachimi Decky plugin: ${error}`);
    }
  }
  ...
  public static async checkPluginVersion(): Promise<{
    ...
    try {
      ...
      if (!fs.existsSync(HYDRA_DECKY_PLUGIN_LOCATION)) {
        logger.log("Hachimi Decky plugin folder not found, installing...");
      ...
      try {
        if (!fs.existsSync(packageJsonPath)) {
          logger.log(
            "Hachimi Decky plugin package.json not found, installing..."
          );
        ...
        if (isOutdated) {
          logger.log(
            `Hachimi Decky plugin is outdated. Current: ${currentVersion}, Expected: ${releaseInfo.version}`
          );
        ...
        logger.error(`Error checking Hachimi Decky plugin version: ${error}`);
        return {
          exists: false,
          outdated: true,
          currentVersion: null,
