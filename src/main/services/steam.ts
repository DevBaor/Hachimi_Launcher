...
const fetchSteamAppDetailsResponse = async (
  objectId: string,
  language: string
  ...
  const url = `https://store.steampowered.com/api/appdetails?${searchParams.toString()}`;
  const response = await net.fetch(url, {
    headers: {
      "user-agent": "Hachimi Launcher",
      accept: "application/json,text/plain,*/*",
    },
  });

  if (!response.ok) {
    logger.error("Steam app details request failed", {
      objectId,
      language,
  ...
  return (await response.json()) as SteamAppDetailsResponse;
};
...
export const getSteamAppDetails = async (
  ...
  return fetchSteamAppDetailsResponse(objectId, language)
    .then((payload) => {
      const appDetails = payload?.[objectId];
