...
const binaryNameByPlatform: Partial<Record<NodeJS.Platform, string>> = {
  darwin: "hachimi-python-rpc",
  linux: "hachimi-python-rpc",
  win32: "hachimi-python-rpc.exe",
};
  ...
  public static async spawn(
    ...
    if (app.isPackaged) {
      ...
      if (!fs.existsSync(binaryPath)) {
        const shouldNotify = Boolean(initialDownload);

        if (shouldNotify) {
          dialog.showErrorBox(
            "Download Service Missing",
            "Hachimi Python Instance binary not found.\n\nDownloads that require the native service will be disabled. Please rebuild the app with the Python RPC binary or check if Windows Defender removed it."
          );
        } else {
          pythonRpcLogger.warn(
            "[PythonRPC] Binary missing; running without the native download service."
          );
        }
