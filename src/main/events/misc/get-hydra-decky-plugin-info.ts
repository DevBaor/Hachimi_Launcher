...
const getHydraDeckyPluginInfo = async (
  ...
  try {
    ...
    if (!fs.existsSync(HYDRA_DECKY_PLUGIN_LOCATION)) {
      logger.log("Hachimi Decky plugin not installed");
      return {
        installed: false,
        version: null,
    ...
    if (!fs.existsSync(packageJsonPath)) {
      logger.log("Hachimi Decky plugin package.json not found");
      return {
        installed: false,
        version: null,
        path: HYDRA_DECKY_PLUGIN_LOCATION,
    ...
    logger.log(
      `Hachimi Decky plugin installed, version: ${version}, expected: ${expectedVersion}, outdated: ${outdated}`
    );
