...
app.whenReady().then(async () => {
  electronApp.setAppUserModelId("gg.hachimi.launcher");
  ...
  const isRunDeepLink =
    deepLinkArg?.startsWith(`${PROTOCOL}://run`) ||
    deepLinkArg?.startsWith(`${LEGACY_PROTOCOL}://run`);
...
app.on("second-instance", (_event, commandLine) => {
  ...
  const isRunDeepLink =
    deepLink?.startsWith(`${PROTOCOL}://run`) ||
    deepLink?.startsWith(`${LEGACY_PROTOCOL}://run`);
